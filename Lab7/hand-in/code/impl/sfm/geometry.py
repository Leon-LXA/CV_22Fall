...
def EstimateEssentialMatrix(K, im1, im2, matches):
    ...
    normalized_kps1 = np.c_[im1.kps, np.ones(im1.kps.shape[0])] @ (np.linalg.inv(K).transpose())
    normalized_kps2 = np.c_[im2.kps, np.ones(im2.kps.shape[0])] @ (np.linalg.inv(K).transpose())
    ...
    for i in range(matches.shape[0]):
        # TODO
        # Add the constraints
        u1 = normalized_kps1[matches[i, 0], 0]
        v1 = normalized_kps1[matches[i, 0], 1]
        u2 = normalized_kps2[matches[i, 1], 0]
        v2 = normalized_kps2[matches[i, 1], 1]
        constraint_matrix[i] = np.array([u1 * u2, u1 * v2, u1, v1 * u2, v1 * v2, v1, u2, v2, 1])
...
def TriangulatePoints(K, im1, im2, matches):
    ...
    # pointsZ = points3D[:, 2]
    test = np.expand_dims(t1, 1)
    T_1_W = np.append(R1.transpose(), -R1.transpose() @ np.expand_dims(t1, 1), 1)
    points3D_homo = np.c_[points3D, np.ones(points3D.shape[0])]
    points3D_cam1 = points3D_homo @ T_1_W.transpose()
    pointsZ = points3D_cam1[:, 2]
    im1_corrs = im1_corrs[np.where(pointsZ > 0)]
    im2_corrs = im2_corrs[np.where(pointsZ > 0)]
    points3D = points3D[np.where(pointsZ > 0)]

    # Filter points behind the second camera
    if points3D.shape[0] > 0:
        T_2_W = np.append(R2.transpose(), -R2.transpose() @ np.expand_dims(t2, 1), 1)
        points3D_homo = np.c_[points3D, np.ones(points3D.shape[0])]
        points3D_cam2 = points3D_homo @ T_2_W.transpose()
        pointsZ = points3D_cam2[:, 2]
        im1_corrs = im1_corrs[np.where(pointsZ > 0)]
        im2_corrs = im2_corrs[np.where(pointsZ > 0)]
        points3D = points3D[np.where(pointsZ > 0)]
...
def EstimateImagePose(points2D, points3D, K):
    ...
    normalized_points2D = np.c_[points2D, np.ones(points2D.shape[0])] @ (np.linalg.inv(K).transpose())
...
def TriangulateImage(K, image_name, images, registered_images, matches):
    ...
    newimage_corrs = []
    offset = 0
    for i, image_reg_name in enumerate(registered_images):
        image_reg = images[image_reg_name]
        e_matches = GetPairMatches(image_name, image_reg_name, matches)
        points3D_new, im1_corrs, im2_corrs = TriangulatePoints(K, image, image_reg, e_matches)
        points3D = np.append(points3D, points3D_new, 0)
        corrs[image_reg_name] = [im2_corrs, offset, offset + points3D_new.shape[0]]
        offset = offset + points3D_new.shape[0]
        newimage_corrs = np.append(newimage_corrs, im1_corrs).astype(np.int32)
    corrs[image_name] = [newimage_corrs, 0, offset]
