...
from impl.sfm.geometry import DecomposeEssentialMatrix, EstimateEssentialMatrix, TriangulatePoints, TriangulateImage, \
    EstimateImagePose
...
def main():
    ...
    for i in range(4):
        ...
        e_im2.SetPose(np.transpose(possible_relative_poses[i][0]),
                      -np.transpose(possible_relative_poses[i][0]) @ possible_relative_poses[i][1])
    ...
    e_im2.SetPose(np.transpose(possible_relative_poses[best_pose][0]), -np.transpose(possible_relative_poses[best_pose][0]) @ possible_relative_poses[best_pose][1])
    e_im1.SetPose(np.eye(3), np.zeros(3))
    ...
    while len(registered_images) < len(images):
        ...
        #     if image_name == "0001.png":
        #         break
        # break
    print("Total 3D points number is:"+str(points3D.shape[0]))
    # Visualize
    fig = plt.figure()
