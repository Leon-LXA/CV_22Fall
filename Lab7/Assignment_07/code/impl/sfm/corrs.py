...
def UpdateReconstructionState(new_points3D, corrs, points3D, images):
    ...
    len_unupdated = points3D.shape[0]
    points3D = np.append(points3D, new_points3D, 0)

    for im_name in corrs:
        list_origin = list(range(corrs[im_name][1], corrs[im_name][2]))
        list_new = [i + len_unupdated for i in list_origin]
        images[im_name].Add3DCorrs(corrs[im_name][0], list_new)
